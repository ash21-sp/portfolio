...
后台服务器由 **macOS 系统守护**（launchd）：开机自动启动、崩溃几秒内自动重启，平时**不用管它**。

- 管理页面：<http://localhost:3210/admin>（同一地址的 `/` 是网站预览）
- 编辑内容 → 点 **保存** → 点 **发布上线**，Vercel 约 1–2 分钟后自动更新线上网站
- `启动后台.command`：确认服务在运行并打开管理页面（服务起不来时它会自动清缓存重试）
- `停止后台.command`：临时停掉后台服务（网站本身不受影响，一直在线）
- `安装守护服务.command`：仅在首次使用/换电脑/升级 Node 后运行一次

> 服务日志在 `~/.portfolio-admin.log`；后台只在本地可访问，线上站点没有 `/admin`。
